...
class ResNet(nn.Module):
    def __init__(self, block, layers=(3, 4, 23, 3), inchannels=1):
        self.inplanes = 64
        self.inchannels = inchannels
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(self.inchannels, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def resnet18(pretrained=True, inchannels=1):
    model = ResNet(BasicBlock, [2, 2, 2, 2], inchannels)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(pretrained=True, inchannels=3):
    model = ResNet(BasicBlock, [3, 4, 6, 3], inchannels)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(pretrained=True, inchannels=3):
    model = ResNet(Bottleneck, [3, 4, 6, 3], inchannels)
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(pretrained=True, in_channels=3):
...
def resnet152(pretrained=True, in_channels=3):
