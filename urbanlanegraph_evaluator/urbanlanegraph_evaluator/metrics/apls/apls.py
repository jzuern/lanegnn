...
def execute_apls(gt_list, gp_list,
            ...
            travel_time_key='travel_time_s',
            max_files=1000,
            linestring_delta=10,#50,
            is_curved_eps=10**3,
            max_snap_dist=5,#4,
            max_nodes=500,
            n_plots=10,
            min_path_length=20,#10,
            allow_renaming=True,
            verbose=True,
            super_verbose=False):
    """
...
if __name__ == '__main__':
    ...
    print("calculate APLS")

    g_gt_file = '/PATH_TO_ANNOTATIONS/gt.gpickle'
    g_pred_file = '/PATH_TO_PREDICTIONS/pred.pkl'

    with open(g_gt_file, 'rb') as f:
        g_gt = pickle.load(f)

    with open(g_pred_file, 'rb') as f:
        g_pred = pickle.load(f)
        g_pred = g_pred["austin"]["test"][41]

    # Filter Agg based on GT graph closeness
    g_pred = filter_graph(target=g_gt, source=g_pred, threshold=50)

    g_pred = adjust_node_positions(g_pred, x_offset=14021, y_offset=56605)
    g_gt = adjust_node_positions(g_gt, x_offset=14021, y_offset=56605)

    g_gt = prepare_graph(g_gt)
    g_pred = prepare_graph(g_pred)

    gt_list = [g_gt]
    gp_list = [g_pred]

    apls_dict = execute_apls(gt_list, gp_list, verbose=False)
    print(apls_dict)
