...
def main():
    ...
    # Load the graphs
    for city in ["austin", "detroit", "miami", "paloalto", "pittsburgh", "washington"]:

        # load annotation pickle files
        annotation_tile_names_train = glob(os.path.join(opt.dataset_root, city, "tiles", "train", "*.gpickle"))
        annotation_tile_names_eval = glob(os.path.join(opt.dataset_root, city, "tiles", "eval", "*.gpickle"))

        print("Found {} training tiles and {} evaluation tiles".format(len(annotation_tile_names_train), len(annotation_tile_names_eval)))

        # load images
        print("Loading aerial image for city:", city)
        aerial_image_name = os.path.join(opt.dataset_root, city, "{}.png".format(city))
        aerial_image = np.asarray(Image.open(aerial_image_name))

        print("Loading direction image for city:", city)
        direction_image_name = os.path.join(opt.dataset_root, city, "{}_direction.png".format(city))
